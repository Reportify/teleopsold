// Circle Vendor Management Service
import api from "./api";

export interface VendorRelationship {
  id: string;
  circle_tenant?: string;
...
export interface VendorInviteRequest {
  vendor_name: string; // Used for TenantInvitation, not stored in CircleVendorRelationship
  vendor_code: string;
  vendor_email: string; // Used for TenantInvitation, not stored in CircleVendorRelationship
  contact_person_name?: string;
  contact_access_level: string;
...
class CircleVendorService {
  /**
   * Get all vendor relationships for the current circle
   */
  async getVendorRelationships(): Promise<VendorRelationship[]> {
    const response = await api.get("/circle-vendor-relationships/");
  ...
  async getVendorRelationship(id: string): Promise<VendorRelationship> {
    const response = await api.get(`/circle-vendor-relationships/${id}/`);
    return response.data;
  }

  /**
   * Invite a new vendor to the circle
   */
  async inviteVendor(vendorData: VendorInviteRequest): Promise<VendorRelationship> {
    const response = await api.post("/circle-vendor-relationships/", {
  ...
  async updateVendorRelationship(id: string, updateData: VendorUpdateRequest): Promise<VendorRelationship> {
    const response = await api.patch(`/circle-vendor-relationships/${id}/`, updateData);
    return response.data;
  }
  ...
  async approveVendor(id: string): Promise<VendorRelationship> {
    const response = await api.patch(`/circle-vendor-relationships/${id}/`, {
  ...
  /**
   * Get vendor relationship statistics for the circle
   */
  async getVendorStats(): Promise<{
    ...
    try {
      ...
      const stats = {
        ...
        pending_invitations: vendors.filter((v) => v.relationship_status === "Circle_Invitation_Sent").length,
...
export const circleVendorService = new CircleVendorService();
export default circleVendorService;
